...
class Api::TokenController < ApplicationController
  ...
  def intuitTokenAPI #DO THIS FIRST for tokens - load intuits token api from YAML into 'params'
  ...
  def new
    intuitTokenAPI  #loads yaml config and URL into params[]
    load_config
    ...
    if(state == @state.to_s)
      ...
      api(result["access_token"]) # FOR TESTING - uses accesstoken to make a test/example api call
    else
  ...
  def refresh_token #Not really used in our case - TN
    load_config
    url = URI(@exchangeURL)
    queryparams = {
    ...
    puts 'url + headers: ' + url.to_s + '   ' + headers.to_s
